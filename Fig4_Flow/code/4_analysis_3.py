...
from matplotlib import rc as plt_rc, pyplot as plt
...
class VecDraw:
    ...
    def draw(draw, origin, vector, color):
        pen = aggdraw.Pen((*color, VecDraw.ALPHA), VecDraw.STROKE_WIDTH)
...
labels = [
    #"Ovary_Vivo_hCG1h_022223",  # Replace due to lack of particles.
...
make = {
    ...
    "special_figs": True,
    "videos": False,
    "all_videos": True,  # When False, only make the masked videos.
}
...
mask_color = np.array([149, 208, 252], np.uint8)
...
for la in labels:
    ...
    if make["videos"] or make["special_figs"]:
        ...
        dir_frames_regular = mkdir(f"frames/regular_{la}/")
        dir_frames_vectors = mkdir(f"frames/vecs_{la}/")
        dir_frames_vectors_no_mask = mkdir(f"frames/vecs_no_mask_{la}/")

        mask = np.array(Image.open(f"registered/particle_masks/{la}_mask.png"))
        mask = Image.fromarray(mask)
        mask = mask.resize(frame_resize, Image.BICUBIC)
        mask = np.array(mask, np.float64)
        mask_grad = ndimage.gaussian_gradient_magnitude(mask, 1.)
        mask_outline = np.empty((*mask.shape, 4), np.uint8)
        mask_outline[:, :, :3] = mask_color
        mask_outline[:, :, 3] = 1 * mask_grad
        mask_outline = Image.fromarray(mask_outline)

        label_overlay, _ = nrrd.read(f"labelmaps/{la}_track.nrrd", index_order="C")
        ...
        for fi in range(n_frames):
            ...
            if make["videos"] or fi == key_time // t_step and make["special_figs"]:
                ...
                if make["all_videos"]:
                    img.save(dir_frames_regular + f"{fi:04d}.png")
                ...
                img_overlay = Image.fromarray(label_overlay[it])
                img_overlay = img_overlay.resize(frame_resize, Image.BICUBIC)
                img_masked = Image.alpha_composite(img, img_overlay)
                img_masked = Image.alpha_composite(img_masked, mask_outline)
                img_masked = img_masked.convert("RGB")
                draw_masked = aggdraw.Draw(img_masked)
                draw_orig = aggdraw.Draw(img)
                ...
                for h, v, c in zip(heads, vecs, colors):
                    ...
                    VecDraw.draw(draw_masked, h, v, c)
                draw_masked.flush()
                draw.flush()
            if make["videos"]:
                img_masked.save(dir_frames_vectors + f"{fi:04d}.png")
                if make["all_videos"]:
                    img.save(dir_frames_vectors_no_mask + f"{fi:04d}.png")
            ...
            if fi == key_time // t_step and make["special_figs"]:
                ...
                img_overlay.save("fig/" + key_particles_path)
                mask_outline.save("fig/" + key_mask_path)
                dwg = svgwrite.Drawing(f"fig/fastest_{la}.svg")
                dwg.add(dwg.image(key_image_path, size=frame_resize,
                                  preserveAspectRatio="none"))
                dwg.add(dwg.image(key_particles_path, size=frame_resize,
                                  preserveAspectRatio="none"))
                dwg.add(dwg.image(key_mask_path, size=frame_resize))
        ...
        if make["videos"]:
            ...
            run_ffmpeg(dir_frames_vectors, "vecs_" + la, crf=8)
            if make["all_videos"]:
                run_ffmpeg(dir_frames_regular, "regular_" + la, crf=15)
                run_ffmpeg(dir_frames_vectors_no_mask, "vecs_no_mask_" + la, crf=8)
...
if make["summary_figs"]:
    line_colors = "blue red black".split()
    shade_colors = "cyan pink grey".split()
    titles = ["hCG 1h (COCs in Ovary)",
              "hCG 13h (COCs in Bursa)",
              "hCG 17h (COCs in Ampulla)"]
    ...
    for i in range(3):
        ...
        for k in range(3):
            ...
            ax.plot(t, speed_avg_f, color=line_colors[k])
            ax.fill_between(
                t,
                speed_avg_f - 2*speed_se_f,
                speed_avg_f + 2*speed_se_f,
                color=shade_colors[k], label="_nolegend_")
    ...
    for i in range(3):
        ...
        for j, body in enumerate(parts["bodies"]):
            body.set_edgecolor(line_colors[j])
            body.set_facecolor(shade_colors[j])
